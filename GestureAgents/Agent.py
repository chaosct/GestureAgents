...
class Agent(object):
    ...
    def _can_confirm(self):
        ...
        if not self._recognizer_complete:
            return False  # if there is no recognizer completed it can't be confirmed
        if self.completed:
            return False  # if it's already confirmed it can't be confirmed
        if not self._recognizers_acquired:
            return True  # if there are no competitors acquired it can be confirmed
