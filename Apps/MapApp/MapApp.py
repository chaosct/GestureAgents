...
class MapApp:
    ...
    def draw(self):
        ...
        GL.glTexCoord2f(0, 0)
        GL.glVertex2f(0, self.textureh)

        # Top Right Of The Texture and Quad
        GL.glTexCoord2f(1, 0)
        GL.glVertex2f(self.texturew, self.textureh)

        # Bottom Right Of The Texture and Quad
        GL.glTexCoord2f(1, 1)
        GL.glVertex2f(self.texturew, 0)
        GL.glEnd()
    ...
    def newAgentTap(self, Tap):
        posinmap = tr.inverse_matrix(
            self.tmatrix).transpose().dot(Tap.pos + (0, 1))
